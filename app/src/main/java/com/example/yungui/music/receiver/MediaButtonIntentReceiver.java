...
package com.example.yungui.music.receiver;
 ...
 * Triple press: previous track  3击
 * Long press: voice search  长按
 */
public class MediaButtonIntentReceiver extends WakefulBroadcastReceiver {
    private static final boolean DEBUG = false;
    private static final String TAG = "ButtonIntentReceiver";

    private static final int MSG_LONGPRESS_TIMEOUT = 1;//长安
    private static final int MSG_HEADSET_DOUBLE_CLICK_TIMEOUT = 2;//双击

    private static final int LONG_PRESS_DELAY = 1000;//长按延时
    private static final int DOUBLE_CLICK = 800;//双击间隔时间
    ...
    private static void startService(Context context, String command) {
        ...
        intent.putExtra(MediaService.CMDNAME, command);
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        if (AudioManager.ACTION_AUDIO_BECOMING_NOISY.equals(intentAction)) {
            ...
            final int keycode = event.getKeyCode();//事件的类型
            final int action = event.getAction();//事件的行为
            final long eventTime = event.getEventTime();//事件是发生时间
